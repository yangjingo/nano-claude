...
def build_parser() -> argparse.ArgumentParser:
    ...
    buddy_parser = subparsers.add_parser(
        "buddy", help="roll a random Zelda champion buddy"
    )
    buddy_parser.add_argument(
        "--registry", action="store_true", help="show all champions registry"
    )

    # Memory commands
    memory_parser = subparsers.add_parser("memory", help="manage persistent memories")
    memory_parser.add_argument(
        "action",
        choices=["list", "show", "add", "delete", "summary"],
        default="list",
        nargs="?",
        help="memory action",
    )
    memory_parser.add_argument("name", nargs="?", help="memory name for show/delete")
    memory_parser.add_argument(
        "--type",
        choices=["user", "feedback", "project", "reference"],
        default="user",
        help="memory type for add",
    )
    memory_parser.add_argument("--description", help="memory description for add")
...
def main(argv: list[str] | None = None) -> int:
    ...
    if args.command == "memory":
        ...
        from ..memory.index import rebuild_index

        storage = LocalStorage()

        if args.action == "summary":
            print(memory_summary(storage))
            return 0

        if args.action == "list":
            entries = load_memories(storage)
            if not entries:
                print("No memories found.")
                print("")
                print("Usage:")
                print(
                    '  nano-claude memory add --type user --description "..." --content "..."'
                )
                return 0
            print(f"Memory Index ({len(entries)} entries)")
            print("")
            for entry in entries:
                print(f"  [{entry.type.value}] {entry.name} — {entry.description}")
            return 0

        if args.action == "show":
            if not args.name:
                print("Error: memory name required")
                return 1
            entry = get_memory(args.name, storage)
            if not entry:
                print(f"Memory not found: {args.name}")
                return 1
            print(f"Name: {entry.name}")
            print(f"Type: {entry.type.value}")
            print(f'Created: {entry.created.strftime("%Y-%m-%d")}')
            if entry.updated:
                print(f'Updated: {entry.updated.strftime("%Y-%m-%d")}')
            print("")
            print(entry.content)
            return 0

        if args.action == "add":
            if not args.description or not args.content:
                print("Error: --description and --content required")
                return 1
            type_map = {
            ...
            name = args.name or f"{args.type}_memory"
            entry = save_memory(
                name=name,
            ...
            print(f"Created: {entry.name} ({entry.type.value})")
            return 0
        ...
        if args.action == "delete":
            if not args.name:
                print("Error: memory name required")
                return 1
            deleted = delete_memory(args.name, storage)
            if deleted:
