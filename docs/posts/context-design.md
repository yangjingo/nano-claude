...
```
有效窗口:   180,000 tokens (200k - 20k 预留)
自动压缩:   167,000 tokens (180k - 13k 缓冲)
警告阈值:   147,000 tokens (167k - 20k 警告)
阻塞阈值:   177,000 tokens (180k - 3k 手动缓冲)
```
...
4. `microcompact_tool_result()` — 时间基础清理
5. 工具结果截断
6. `_summarize_tool_result()` — 按工具类型生成摘要

### Phase 3: 完整压缩
7. Token 计数器（基于 response.usage）
8. 自动压缩阈值检测
9. `full_compact()` — 对话摘要 + 最近 N 轮保留

### Phase 4: Skill/MCP 集成
10. Skill prompt 动态注入
11. MCP tool definition 按需加载
12. MCP tool definition 压缩

### Phase 5: 透明性
13. Compact 事件通知用户
14. 压缩统计（节省了多少 tokens）
15. 展开查看原始内容
